...
public class LayoutManagerMixin {
    ...
    /**
     * Transforms every widget on the screen during open/close animation such that widgets on the left move to/from the
     * left, widgets
     * on the right move to/from the right and widgets which are horizontally centered move towards/away from their
        ...
        if (doAnimate) {
            ...
            if (val > 0) {
                ...
                if (lPos < 0 && rPos < 0) {
                    ...
                    // the * 4 makes the animation slightly faster to fit with the rest (they travel a much shorter
                    // distance)
